...
func (receiver *TranscodePostMedia) Handle(args ...any) error {
	if len(args) != 1 {
		return errors.New("transcode_post_media requires a media ID")
	}
	mediaID, ok := args[0].(int64)
	if !ok || mediaID <= 0 {
		return fmt.Errorf("invalid media ID: %v", args[0])
	}
	var media models.PostMedia
	if err := db.DB.First(&media, mediaID).Error; err != nil {
		return err
	}
	if media.MediaType != models.MediaTypeVideo {
		return nil
	}
	parsed, err := url.Parse(media.MediaURL)
	if err != nil {
		return err
	}
	...
	inputPath := filepath.Join(uploadRoot, filename)
	if filepath.Dir(inputPath) != uploadRoot {
		return errors.New("unsafe media path")
	}
	base := strings.TrimSuffix(filename, filepath.Ext(filename))
	outputName := base + ".optimized.mp4"
	...
	if output, err := command.CombinedOutput(); err != nil {
		_ = db.DB.Model(&media).Update("processing_status", "failed").Error
		if len(output) > 500 {
	...
	if err := db.DB.Model(&media).Updates(map[string]any{"media_url": newURL, "processing_status": "ready"}).Error; err != nil {
