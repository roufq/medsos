...
# 1. Update routes/web.go
routes_path = "c:/laragon/www/medsos/routes/web.go"
with open(routes_path, 'r', encoding='utf-8') as f:
    routes_content = f.read()

if 'web.Get("admin/posts"' not in routes_content:
...
# 2. Update user_controller.go
ctrl_path = "c:/laragon/www/medsos/app/http/controllers/user_controller.go"
with open(ctrl_path, 'r', encoding='utf-8') as f:
    ctrl_content = f.read()

if 'func (h *UserController) ShowAdminPostsWeb' not in ctrl_content:
    new_method = """
...
"""
    # Append it right before ShowAdminModerationWeb
    ctrl_content = ctrl_content.replace(
        'func (h *UserController) ShowAdminModerationWeb',
        new_method + '\nfunc (h *UserController) ShowAdminModerationWeb'
    )
    with open(ctrl_path, 'w', encoding='utf-8') as f:
        f.write(ctrl_content)

# 3. Create dashboard_admin_posts.go
view_path = "c:/laragon/www/medsos/resources/views/backend/dashboard_admin_posts.go"
view_content = """package backend
	...
	for _, p := range posts {
		...
		postRows = append(postRows, dom.Tr(
			...
			dom.Td(
				...
				dom.Button(
					[]dom.Attr{dom.Class("p-2 text-text-secondary hover:text-error transition-colors")},
...
print("Posts dashboard created and configured successfully!")
